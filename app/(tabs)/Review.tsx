...
const boundsFromChange = (change: StopSignChange): JsonBBox | undefined => {
  ...
  return minlat === undefined ||
    minlon === undefined ||
    maxlat === undefined ||
    maxlon === undefined
    ? undefined
    : { minlat, minlon, maxlon, maxlat };
};
...
function SubmitStop(params: { change: StopSignChange }) {
  ...
  useOsmData({
    loadingBounds: bounds,
    intersections,
    setIntersections,
    isWaySelected,
    withNewWays,
    interestingWays: params.change.selectedWays,
    sameRoads,
    setSameRoads,
  });
  if (
    !queryWays.data ||
    !queryWays.data.centrelines ||
    !queryWays.data.centrelines.features
  )
...
export default function Review() {
  ...
  const result = qReviewable.isSuccess ? (
    qReviewable.data.map((change) => {
      ...
      return (
        <ListItem onPress={onPress} key={change.id}>
          <ListItem.Content>
            <ListItem.Title>
              {change.commentary1 ||
                (change.type === "addStopSign"
                  ? "*Add Stop Sign"
                  : change.type)}
            </ListItem.Title>
            <ListItem.Subtitle>
              {change.commentary2
                ? `${change.commentary2} at ${formattedDate}`
                : formattedDate}
            </ListItem.Subtitle>
            {submitting[change.id] && change.type === "addStopSign" && (
              <SubmitStop change={JSON.parse(change.state_extract)} />
            )}
          </ListItem.Content>
          <ListItem.CheckBox
            key={1}
