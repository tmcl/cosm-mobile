...
const isStringRecord = (obj: object): obj is Record<string, string> => {
	return !Object.getOwnPropertyNames(obj).some(prop => typeof prop !== "string" || typeof (obj as any)[prop] !== "string")
}
...
const verifyAsStopSign = (change: unknown): State['modes']['addStopSign']['change']|undefined => {
	...
	try {
		...
		const direction = "direction" in change ? (change.direction === undefined || change.direction === "forward" || change.direction === "backward" ? change.direction : fail("direction was wrong")) : undefined
		const selectedWays = "selectedWays" in change && change.selectedWays instanceof Array && !change.selectedWays.some(s => (typeof s !== "string"))
			? change.selectedWays
			: fail("selected ways was wrong")
		const tappedLocation = "tappedLocation" in change && (change.tappedLocation === null || typeof change.tappedLocation === "object")
			? (change.tappedLocation === null || isNewTappedLocation(change.tappedLocation) || isTargetNode(change.tappedLocation) ? change.tappedLocation :  fail("tapped loc 2"))
			: fail("tapped location was wrong")
		const highwayLocation = "highwayLocation" in change && (change.highwayLocation === null || typeof change.highwayLocation === "object")
		? (change.highwayLocation === null || isNewHighwayLocation(change.highwayLocation) || isTargetNode(change.highwayLocation) ? change.highwayLocation :  fail("highway loc 2"))
...
type ActionAddStopSign = { action: "add stop sign", tappedLocation: null } | { action: "add stop sign", tappedLocation: GeoJSON.Point, newId: `new-${number}` }
			...
			switch(action.mode) {
				...
				case "addStopSign": {
					...
					if (modalAction.point.properties.ways.length === 0
						...
						return {...state, modes: {...state.modes, addStopSign: {...state.modes.addStopSign,
									change: {...state.modes.addStopSign.change,
						...
						if (!state.modes.addStopSign.change.highwayLocation) {
							return {
								...
								modes: {
									...state.modes,
									addStopSign: {...state.modes.addStopSign, change: {...state.modes.addStopSign.change, highwayLocation: modalAction.point, selectedWays: [...modalAction.point.properties.ways]}}
								}
							}
						} else if ("type" in state.modes.addStopSign.change.highwayLocation && state.modes.addStopSign.change.highwayLocation.id === modalAction.point.id) {
							return {
								...state,
								modes: {
									...state.modes,
									addStopSign: {...state.modes.addStopSign, change: {...state.modes.addStopSign.change, highwayLocation: null}}
								}
							}
						} else {
							return state
						...
						case "select ways": {
							...
							const selectedWays = select
								? Object.keys(Object.fromEntries([/* ...state.selectedWays,*/ ...modalAction.ways].map(f => [f, true]))).sort()
							...
							return {
								...
								modes: {
									...state.modes,
									addStopSign: {...state.modes.addStopSign, change: {...state.modes.addStopSign.change, selectedWays, highwayLocation}}
								}
							}
						}
						case "add stop sign": {
							return {...state, modes: {...state.modes, addStopSign: {...state.modes.addStopSign,
										change: {...state.modes.addStopSign.change,
											tappedLocation: modalAction.tappedLocation ? {type: "new", newId: modalAction.newId, point: modalAction.tappedLocation} : null }}}}
						}
						default: {
			...
			switch (action.query) {
				...
				case "saveNewUserDataChanges": {
					...
					if (action.queryState.status === "success" && data) {
						...
						if (ismode(type) && "changeId" in updateState.modes[type] && updateState.modes[type].changeId === undefined) {
					...
					const process = <T extends 'sameRoads'|'intersections'>(query: T) => {
						if (action.queryState.status === "success" && action.queryState.data) {
							...
							for (const key in data) {
								if(anyModeSelectsWay(state, key) || state.queries.queryWays.data?.casings?.features.find(f => f.id === key) || unincluded++ < 5 ) {
								} else {
									delete data[key]
								}
							}
...
function buildStatusString(debouncers: boolean[], queries: PartialRecord<string, MutationState<unknown, unknown>|QueryState<unknown, unknown>>) {
...
export default function MainPage() {
	...
	console.log("params", params, state.neededForLoading, state.queries.neededForLoading, state.modes.addStopSign, state.mode)
	...
	const editableSign = state.mode === "addStopSign"
		? (state.modes.addStopSign.change.tappedLocation?.type === "Feature"
			? state.modes.addStopSign.change.tappedLocation.geometry : state.modes.addStopSign.change.tappedLocation?.point) : undefined
	...
	useDispatchingQuery (
		(queryState: QueryState<unknown, SavedChangeSet|null>) => dispatch({action: "set query", query: "neededForLoading", queryState}),
		{
	...
	const qSaveUpdateUserChanges = useDispatchingMutation(
		(queryState: MutationState<unknown, number>) => dispatch({action: "set query", query: "saveUpdateUserDataChanges", queryState}),
		{
	...
	const debSaveUpdateUserChanges = useDebouncedCallback((args: {id: number, args: MyChangeSet}) => {
	...
	useEffect(() => {
		console.log("there has been observed a change to ", [state.mode, state.modes[state.mode]])
			...
			if ("changeId" in modeSettings && typeof modeSettings.changeId === "number" && state.mode !== 'browse') {
				...
				debSaveUpdateUserChanges({id: modeSettings.changeId, args: {
					type: state.mode,
	...
	const qSaveNewUserChanges = useDispatchingMutation(
		(queryState: MutationState<unknown, SavedChangeSet>) => dispatch({action: "set query", query: "saveNewUserDataChanges", queryState}),
		{
	...
	useEffect(() => {
		...
		if("changeId" in modeSettings && modeSettings.changeId === undefined && state.mode !== 'browse') {
			qSaveNewUserChanges.mutate({type: state.mode, change: constructedSign.changes, state_extract: state.modes[state.mode].change})
		}
	...
	const setTappedLocation = (tappedLocation: GeoJSON.Point|null, toggle = false) => {
		switch(state.mode) {
			case "addStopSign":
				const oldLoc = toggle && state.modes.addStopSign.change.tappedLocation ? null : state.modes.addStopSign.change.tappedLocation
				const loc = toggle && state.modes.addStopSign.change.tappedLocation ? null : tappedLocation
				return dispatch({action: "modal", mode: "addStopSign", modalAction: loc ? {action: "add stop sign", tappedLocation: loc, newId: oldLoc && oldLoc.type === "new" ? oldLoc.newId : `new-${make_id()}`} : {action: "add stop sign", tappedLocation: loc}})
			case "browse":
				return;
			default:
	...
	const onPressWay = (event: OnPressEvent) => {
		...
		switch(state.mode) {
			case "addStopSign":
				return dispatch({action: "modal", mode: "addStopSign", modalAction: {action: "select ways", ways: event.features.map(i => i.id!.toString()), select: "toggle"}})
			case "browse":
				return
			default:
				const c: never = state.mode
	...
	const fabButtonPress = () => {
		switch (state.mode) {
			...
			case "addStopSign": return console.log("need to support adding signs")
		}
	}
			...
			const
				...
				if (features.length === 1) {
					...
					if (feature.geometry.type === "Point") {
						dispatch({action: "modal", mode: "addStopSign", modalAction: {action: "select interesting point", point: feature as TargetNode}})
					}
	...
	return (
		<View
			style={styles.page}
		>
			<View style={{flexWrap: "wrap", alignContent: "center", flexDirection: "row", gap: 4, paddingLeft: 2, paddingRight: 2}}>{statusString}</View>
			<MapLibreGL.MapView
			...
			>
				...
				<MapLibreGL.Camera
					...
					followUserLocation
				/>
				{editableSign && <MapLibreGL.PointAnnotation key={radians} ref={refTappedLoc} onDragEnd={e => setTappedLocation(e.geometry)} id="centrepoint" coordinate={editableSign.coordinates} draggable={true} >
					<View>
						<Svg.Svg  height="25" width="25" viewBox="0 0 100 100" >
							...
							{radians !== undefined ?
								...
								<Svg.Line
									...
									x1={50} y1={50}
									x2={50 + (43) * Math.cos(radians-Math.PI/2)} y2={50 + (43) * Math.sin(radians-Math.PI/2)}></Svg.Line>
								</>
								:  false }
						</Svg.Svg>
					</View>
			...
			{(constructedSign.notes.length > 0 || constructedSign.changes.length > 0) &&
				<View style={{left: 0, top: 0, margin: 16, padding: 16, backgroundColor: "#f0edeecc", position: "absolute"}}>
					...
					{constructedSign.changes.map((note, i) => <Text style={{fontFamily: "monospace"}} key={i}>{JSON.stringify(note)}</Text>)}
				</View>
			}
			<View
				style={{
...
const fabFromMode = (mode: Mode): [string|undefined, IconNode, {icon: IconNode, text: string|undefined, mode: Mode}[]] => {
	switch (mode) {
		case "browse":
			return [undefined, {name: "menu", color: "white", type: "material-community"}, [{icon: {name: "octagon", color: "white", type: "material-community"}, text: "Stop", mode: "addStopSign"}]]
		case "addStopSign":
...
const selectedInterestingPointsForMode = (state: State): string[] => {
	switch (state.mode) {
		case "browse": return []
		case "addStopSign": return [
			state.modes.addStopSign.change.tappedLocation?.type === "Feature" && state.modes.addStopSign.change.tappedLocation.id?.toString(),
			state.modes.addStopSign.change.highwayLocation && "type" in state.modes.addStopSign.change.highwayLocation && state.modes.addStopSign.change.highwayLocation.id?.toString()
		].flatMap(m => m ? [m] : [])
	}
}
...
const interestingNodesParamsFromMode = (state: State): undefined|Omit<InterestingNodesParams, 'minlon'|'minlat'|'maxlon'|'maxlat'> => {
	switch (state.mode) {
		...
		case "addStopSign": return {$needles: [{"highway": "stop"}, {"traffic_sign": "stop"}, {"highway": "give_way"}, {"highway": "giveway"}]}
	}
}
...
const inferDirectionAndAngle = (
	...
	: {wayId: undefined|WayId /* direction is relative to wayId */, angle: undefined|number, direction: undefined|"forward"|"backward"} => {
...
const modalNotes = (
	...
	switch (state.mode) {
		case "addStopSign":
			...
			if(tappedLocation && highwayLocation && inferredSignDirectionAndAngle().wayId !== inferredHighwayDirectionAndAngle().wayId) {
			...
			if (inferredHighwayDirectionAndAngle().direction !== highwayDirection) {
				notes.push("Note: something might be off because a different direction has been inferred than the configured direction")
			}

			if(tappedLocation && highwayLocation && inferredSignDirectionAndAngle().direction !== inferredHighwayDirectionAndAngle().direction) {
			...
			if(!tappedLocation) {
				...
				signFaceAngle = inferredAngle === undefined ? undefined : Math.round(bound(inferredAngle + signTypeAngle + (apparentDirection === "backward" ? 180 : 0), 0, 360))
				const signTags: PartialRecord<string, string> = rmUndef({direction: signFaceAngle?.toString(), "traffic_sign": "stop"})
				if(tappedLocation.type === "new") {
					notes.push("Adding a new stop sign")
					changes.push({object: {type: "node", position: tappedLocation.point.coordinates, node_id: tappedLocation.newId}, set_tags: signTags})
			...
			if(!highwayLocation) {
				...
				if("type" in highwayLocation) {
					if(highwayDirection === directionFromString(highwayLocation.properties.tags?.direction)) {
						...
						notes.push("Amending an existing sign")
						changes.push({object: {type: "node", node_id: highwayLocation.id as `${number}`}, set_tags: {...highwayLocation.properties.tags, "highway": "stop", ...(highwayDirection ? {"direction": highwayDirection} : {})}})
					}
				} else {
					notes.push("Adding a new stop sign")
				}
...
const allModesSelectedWays = (state: State): WayId[] => {
	...
	return Object.keys(state.modes).flatMap(k => {
		const t: keyof State['modes'] = k as keyof State['modes'] //this happens to be safe due to the logic of the switch - the default returns false
		switch (t) {
			case "addStopSign": {
				return state.modes.addStopSign.change.selectedWays
			}
...
const a_includes_b = (a: PartialRecord<string, string>, b: PartialRecord<string, string>, isIncluded: ((key: string, a_value: string|undefined, b_value: string|undefined) => boolean)|undefined = undefined): boolean => {
	return !Object.entries(b).some(([bkey, bval]) => isIncluded ? isIncluded(bkey, a[bkey], bval) : a[bkey] !== bval)
}

const twoConnectedWaysHaveSameDirection = (wayId1: WayId, wayId2: WayId, wayInfo: PartialRecord<WayId, IWay>): boolean|null => {
...
type MyChangeSet_<M extends Mode, Y, X extends State['modes'][M] & {change: Y}> = {type: M, change: Change[], state_extract: X['change']}
