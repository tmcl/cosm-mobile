...
let
  ...
  overlay = (
    hsSelf: hsSuper: {
      ...
      #  }  ; } hsSuper.ekg-json));
      stopwatch = final.haskell.lib.compose.doJailbreak (
      ...
      servant-typescript = final.haskell.lib.compose.doJailbreak (
        final.haskell.lib.compose.unmarkBroken (
          final.haskell.lib.compose.overrideSrc { src = ./tmp/servant-typescript; } hsSuper.servant-typescript
        )
      );
      aeson-typescript = (
        final.haskell.lib.compose.overrideSrc { src = ./tmp/aeson-typescript; } hsSuper.aeson-typescript
      );
      #wkt-geom =
...
in
{
  ...
  gettext =
    if final.stdenv.hostPlatform == final.stdenv.buildPlatform then
      ...
      prev.gettext.overrideAttrs ({
        makeFlags = [
          "CFLAGS=-D_FORTIFY_SOURCE=0"
          "CFLAGS=-D__USE_FORTIFY_LEVEL=0"
        ];
        postPatch = ''
  ...
  libunistring =
    if final.stdenv.hostPlatform == final.stdenv.buildPlatform then
      ...
      prev.libunistring.overrideAttrs ({
        makeFlags = [
          "CFLAGS=-D_FORTIFY_SOURCE=0"
          "CFLAGS=-D__USE_FORTIFY_LEVEL=0"
        ];
  ...
  libspatialite =
    if final.stdenv.hostPlatform == final.stdenv.buildPlatform then
      ...
      prev.libspatialite.overrideAttrs (old: {
        ...
        makeFlags = [
          ...
          "CFLAGS=-I${inputs.libspatialite}/exposqlite"
          "CFLAGS=-D__USE_FORTIFY_LEVEL=0"
          "CFLAGS=-I${final.libunistring.dev}/include"
        ...
        configureFlags = [
          ...
          "--disable-gcp"
          "--enable-freexl=no"
          "--with-geosconfig=${final.geos}/bin/geos-config"
        ];
        buildInputs =
          with final;
          [
  ...
  gradle_8_8 = final.wrapGradle (
    prev.gradle_8.overrideAttrs (rec {
      version = "8.8";
