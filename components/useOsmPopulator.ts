...
import { JsonBBox } from "@/components/types";
import {
...
export default (
  ...
  useDispatchingQuery(setOsmMapQ, {
    ...
    enabled: unknownBoundsDone && !!osmMapArgs,
    queryFn: async () => {
  ...
  const qInsertNodes = useDispatchingMutation(setInsertNodesQ, {
    mutationFn: async (param: { $json: string }) =>
      await queries.current.doInsertNodes(param),
    onSuccess: (changes) => {
      if (changes) {
  ...
  const qUpdateCasings = useDispatchingMutation(seUpdateCasingsQ, {
    mutationFn: async () =>
      (await queries.current.doAddCasingToWays()).changes > 0,
    onSuccess: (data) => {
      console.log("we have updated some casings: ", data);
      if (data) {
        queryClient.invalidateQueries({ queryKey: ["spatialite query ways"] });
        queryClient.invalidateQueries({
          queryKey: ["spatialite", "nearby ways"],
        });
  ...
  const qInsertWays = useDispatchingMutation(setInsertWaysQ, {
    mutationFn: async (param: { $json: string }) =>
  ...
  const qInsertRelatedWays = useDispatchingMutation(setInsertRelatedWaysQ, {
  ...
  const qInsertBounds = useDispatchingMutation(setInsertBoundsQ, {
    mutationFn: async (args: { $json: string; $requestedBounds: JsonBBox }) => {
      await queries.current.doInsertBounds(args);
    },
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["spatialite known bounds", variables.$requestedBounds],
      });
    },
  });
