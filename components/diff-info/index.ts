...
type TurfNearestPoint = GeoJSON.Feature<
  GeoJSON.Point,
  {
    dist: number;
    index: number;
...
export type StopSignChange = {
  type: "add stop sign";
  tappedLocation:
    | { newId: `new-${number}`; point: GeoJSON.Point; type: "new" }
    | TargetNode
    | null;
  highwayLocation:
    | { way: WayId; point: NearestPoint; type: "new" }
    | TargetNode
    | null;
...
export const modalNotes = (
  ...
  let on: string | undefined = selectedWays
    .map((w) => w.properties.tags?.name)
    .filter((f) => !!f)[0];
  ...
  const changesOld: {
    object: OsmObject;
    set_tags: PartialRecord<string, string>;
  }[] = [];
  ...
  const tappedLocationPoint = tappedLocation
    ? tappedLocation.type === "new"
      ? tappedLocation.point
      : tappedLocation.geometry
    : undefined;
  const highwayLocation = addStopSign.highwayLocation;
  const highwayLocationPoint = highwayLocation
    ? highwayLocation.type === "Feature"
      ? highwayLocation.geometry
      : highwayLocation.point
    : undefined;
  ...
  if (!tappedLocation) {
    ...
    const apparentDirection =
      inferredHighwayDirectionAndAngle().direction ||
      inferredSignDirectionAndAngle().direction;
    signFaceAngle =
      inferredAngle === undefined
        ? undefined
        : Math.round(
            bound(
              inferredAngle +
    ...
    if (tappedLocation.type === "new") {
      ...
      sign = "edit";
      if (
        tappedLocation.properties.tags &&
        a_includes_b(tappedLocation.properties.tags, signTags)
      ) {
        notes.push("A sign is selected, but it's all good");
      } else {
        notes.push("Amending an existing sign");
        changesOld.push({
          object: { type: "node", node_id: tappedLocation.id as `${number}` },
          set_tags: signTags,
        });
        osmModify.push({
          tag: "node",
  ...
  if (!highwayLocation) {
    ...
    if (highwayLocation.type === "Feature") {
      ...
      if (
        highwayDirection ===
        directionFromString(highwayLocation.properties.tags?.direction)
      ) {
        notes.push("A stopping point is selected, but it's all good");
      } else {
        const highwayTags = {
          ...highwayLocation.properties.tags,
          highway: "stop",
          ...(highwayDirection ? { direction: highwayDirection } : {}),
        };
        notes.push("Amending an existing sign");
        ...
        changesOld.push({
          ...
          set_tags: highwayTags,
        });
      }
    } else {
      line = "new";
      const street = selectedWays
        .map((w) => w.properties.tags?.name)
        .filter((f) => !!f)[0];
      ...
      changesOld.push({
        object: {
          type: "node",
          position: highwayLocation.point.geometry.coordinates,
          node_id: newNodeId,
        },
        set_tags: signTags,
      });
      ...
      if (selectedWay && waynodes) {
        ...
        changesOld.push({
          object: {
            type: "way",
            way_id: highwayLocation.way as `${number}`,
            nodes: complete.map((m) => m.toString()),
          },
          set_tags: {},
        });
      }
...
const inferDirectionAndAngle = (
  ...
  const ways = selectedWays.map((way) => {
...
const calculateDirectionToNearestIntersection = (
      ...
      ? [
          {
            ...
            geometry: {
              type: "Point",
              coordinates: way.geometry.coordinates[wna.ix],
            },
          },
        ]
      : [];
  ...
  const nearestIntersection = turf.nearestPoint(nearestPointOnLine, {
    type: "FeatureCollection",
    features: wayIntersections,
  });
  ...
  if (
    ...
    return "forward";
  return trueIx <
    wayIntersections[nearestIntersection.properties.featureIndex].properties.ix
    ? "forward"
    : "backward";
};
...
const a_includes_b = (
  a: PartialRecord<string, string>,
  b: PartialRecord<string, string>,
  isIncluded:
    | ((
        key: string,
