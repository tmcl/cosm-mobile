...
export default (params: {
  ...
  const processableState = {
    ...
    sameRoads: params.sameRoads,
  };
  const updatableState = {
    intersections: params.setIntersections,
    sameRoads: params.setSameRoads,
  };
  ...
  useDispatchingQuery(setQueryWays, {
    ...
    placeholderData: (d) => d,
    queryFn: loadingBounds
      ? async () => {
          const result = await queries.current.doQueryWays({
            ...loadingBounds,
            $limit: MAX_FEATURES_QUERY,
          });
