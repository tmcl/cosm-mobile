...
export const mapResult = <T, R>(result: Result<T>, f: (t: T) => R): Result<R> => {
...
export const foldMap = <T, R>(result: Result<T>, f: (t: T) => Result<R>): Result<R> => {
  if ("success" in result) {
...
export type InlineParserSpec<K> = ParserSpec<K> & { xmlname: string } & { if_absent?: "continue anyway" | "default value" }


export const parseInline = <T>(parserSpec: InlineParserSpec<T> ): Parser<T> => (nodes: Node[]): Result<T> => {
...
export const parseString = (nodes: Node[]): Result<string> => {
  return withOneNode<string>(nodes, node => {
    switch (node.nodeType) {
      ...
      case node.TEXT_NODE:
        return typeof node.textContent === "string" ? {success: node.textContent} : {error: "empty string" as const}
      default:
...
export const buildNumber: Builder1<number> = (document: Document, val: number) => {
...
export const buildArray = <T>(builder: Builder<T>): Builder<T[]> => (document: Document, vals: T[]) => {
  return vals.flatMap(val => builder(document, val))
}

export const parseArray = <T>(parser: Parser<T>) => (nodes: Node[]): Result<T[]> => {
  ...
  if (errors.length) {
    return {error: `while parsing array received the following errors:`, argument: errors}
  } else {
    return {success: successes}
  }
}

export const buildObject = <T>(builderObject: BuilderObject<T>): Builder<T> => (document: Document, val: T): [
  {
    attributes: Attr[],
    children: Node[]
  }] => {
...
const handleAbsence = <T>(parser: ParserSpec<T>, nodes: Node[]): Result<T>|null => {
  if (nodes.length === 0) {
...
export const parseObject = <T extends object>(parserObject: ParserObject<T>) => (nodes: Node[]): Result<T> => {
      ...
      const parseValue = (): null|Result<T[Extract<keyof T, string>]> => {
        switch (parser.type) {
          case "element": {
            ...
            Array.from(node.childNodes || []).forEach(n => isElementNode(n) && n.tagName === key && foundNodes.push(n))
            return handleAbsence(parser, foundNodes)
          }
          case "attribute": {
            const foundAttribute = isElementNode(node) && node.attributes.getNamedItem(key)
            return handleAbsence(parser, foundAttribute ? [foundAttribute] : [])
          }
          default:
...
const withOneNode = <T>(nodes: Node[], f: (n: Node) => Result<T>): Result<T> => {
  if (nodes.length < 1) {
...
export const parseRoot = <T>(name: string, parser: Parser<T>) => (doc: Document): Result<T> => {
  ...
  if (root.tagName === name) {
    ...
    return {error: `root element had name ${root.tagName} but we were asked to parse a root of ${name}`}
  }
}
...
const unused = (u: never): never => { throw u }
