...
export const mkNewHighwayNode = (wayId: WayId, wayCentrelines: Record<WayId, Feature<LineString, IWay>>, relativePoint: GeoJSON.Position|GeoJSON.Point): NewPoint|undefined => {
  const closestPoints = Object.entries(wayCentrelines).flatMap(([localWayId, way]) => {
    if (!way) return []
  ...
  .sort((f, g) => f.properties.distance - g.properties.distance )
      [0]
  const bestPoint: NearestPoint = {...closestPoint, properties: {
